...
void PRE_RUN(const shared_ptr<CuCRT_RGSW_CryptoParameters> cp);
...
int main(int argc, char **argv) {
	...
	if(argc !=5){
		...
		cout << argv[0] << "  cyclotomicNumber bits depth fileName\n";
	...
	uint32_t p = 5; //plaintext parameter
...
void PRE_RUN(const shared_ptr<CuCRT_RGSW_CryptoParameters> cp){
	...
	start = currentDateTime();
	auto decrypt = CuRGSW::Decrypt(cipher, kp.secretKey);
	stop = currentDateTime();
	cout << decrypt << "\n";
	...
	start = currentDateTime();
	auto rk = CuRGSW::ReKeyGen(kpNew.publicKey, kp.secretKey);
	stop = currentDateTime();
	...
	start = currentDateTime();
	auto cipherNew = CuRGSW::ReEncrypt(rk, cipher);
	stop = currentDateTime();
	...
	start = currentDateTime();
	auto decryptNew = CuRGSW::Decrypt(cipherNew, kpNew.secretKey);
	stop = currentDateTime();
	cout << decryptNew << "\n";
	std::cout << "time taken for decryption is \t" << (stop-start) << " ms\n";
